...
6. **Plan-mode-enforcer (NOVO, de D3:90–94; refinado em Wave 8-spec / v4.17.0):** Tasks classificadas como MEDIA ou COMPLEXA acionam plan-architect automaticamente antes de qualquer Edit/Write. **Bypass para MEDIA:** flag `--no-plan` explícita + entrada no TRACE.md justificando (`plan_mode_skipped: true`, `plan_override_attempted: true`, `justification: <user input>`). **COMPLEXA sempre roda plan-architect** — `--no-plan` é aceito pelo parser mas ignorado; o flag e a justificativa são logados em TRACE.md (`plan_mode_skipped: false`, `plan_override_attempted: true`, `justification: <user input>`) para auditoria. Razão da assimetria: tarefas COMPLEXA são scope-driven; pular planning nesse nível historicamente correlaciona com regressões. Trigger oficial: classe ∈ {MEDIA, COMPLEXA} AND `--no-plan` ausente OR classe == COMPLEXA (always). Reusa `references/complexity-matrix.md` como SSOT. Verificação: rodar `/pipeline-orchestrator:bugfix [task com domains_touched=2]` em fixture; log mostra plan-architect invocado antes da fase de implementation.
