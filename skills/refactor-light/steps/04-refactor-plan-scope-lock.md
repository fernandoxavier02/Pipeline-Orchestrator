...
Produce the refactor plan and lock its scope with a signed CHANGE_CONTRACT **before any file is touched**. This is the signature gate of the Refactor workflow: `REFACTOR_SCOPE_LOCK` (HARD). It validates that the CHANGE_CONTRACT plan-block exists and is signed, and it ABORTS the run if a forbidden change (new public API surface, changed error codes, changed output format, changed side-effects) is proposed. This is a **mandatory user-facing gate** (`gate_required: true`).

## Why this gate exists

A refactor's whole premise is "behavior does not change". The scope-lock makes that auditable: the allowed/forbidden columns are agreed and signed before the first edit, so the diff-discipline reviewer and the step-6 IDENTICAL check have an explicit contract to enforce against. The gate name is deliberately distinct from the CHANGE_CONTRACT plan-block it consumes, so the audit trail never confuses a gate decision with the plan artifact.
...
- `public_surface`, `restructure_goal` (from step 1)
...
- **Allowed changes:** rename, extract, inline, move, restructure internal data — anything that does not alter the public surface from step 1.
- **Forbidden changes:** new public API surface, changed error codes, changed output format, changed side-effects (these would change observable behavior and are not a refactor).
- **allowed_files / allowed_new_files / forbidden_files:** the exact file scope of the restructure.
- **signed_off:** the flag REFACTOR_SCOPE_LOCK checks.

### 4.2 Draft the refactor steps

List the concrete restructure steps (each a behavior-preserving transformation) so step 5 applies them as a minimal diff. Map each step to allowed-change categories above.

### 4.3 Forbidden-change check (ABORT condition)

Inspect the plan: if any drafted step would introduce a forbidden change (new public API, changed error code, changed output format, changed side-effect), the gate ABORTS — this is not a refactor and must be re-scoped or routed to a different workflow (Feature/Bug Fix). Do NOT proceed to step 5 on ABORT.

### 4.4 AskUserQuestion gate (mandatory — no prose substitute)

Per global rule "Decisoes do Usuario — AskUserQuestion sempre", invoke AskUserQuestion to sign off the contract. The first option is the agent's recommendation labeled `(Recomendado)`:

```
header: "ScopeLock"
question: "Assinar o contrato de mudança e travar o escopo do refactor?"
multiSelect: false
options:
  - label: "Assinar e travar — LOCKED (Recomendado)"
    description: "Contrato fechado: <N> arquivos permitidos, zero mudança de superfície pública. Libera o primeiro toque em arquivo."
  - label: "Revisar o contrato antes de travar"
    description: "Quero ajustar o que pode/não pode mexer antes de liberar a execução."
...
- Append the REFACTOR_SCOPE_LOCK decision to `.pipeline/gate-decisions.jsonl` per enforcement rule 7 (audit log), recording allowed/forbidden columns and the signed flag.

## Done criteria

- CHANGE_CONTRACT plan-block assembled with allowed/forbidden columns + file scope + signed_off flag.
