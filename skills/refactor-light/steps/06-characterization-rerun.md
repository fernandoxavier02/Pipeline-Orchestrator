...
Capture the results as `after_snapshot` using the same shape as `before_snapshot` (test names + pass status + compared values).

### 6.2 Compare before vs after (public surface only)

Compare `after_snapshot` to `before_snapshot`:
- Every public-surface test must have the SAME pass status and the SAME compared values (outputs, error codes, output formats, side-effects).
- Internal coverage-only test changes are EXCLUDED from this comparison — a refactor may legitimately move or rename internal tests. Only the PUBLIC-behavior characterization tests bind to the IDENTICAL check (per the CHANGE_CONTRACT allowed-files public surface).

### 6.3 Verdict

Pre-tester (characterization mode) emits one of:

- **IDENTICAL** (status `CHARACTERIZATION_CONFIRMED`) — every public-behavior characterization test matches the before snapshot exactly. Behavior preserved; proceed.
- **DIVERGED** — at least one public-behavior result differs. The restructure changed observable behavior, which a refactor must not. Record `diverged_details` (which test, before vs after value) and STOP. Two consecutive DIVERGED results trigger the STOP RULE (`stop_rule_max_failures: 2`). The fix is to return to step 5 and correct the restructure so behavior is preserved — NOT to edit the characterization tests to fit the new behavior (that would be `test_weakened_to_fit_implementation`).
...
If `IDENTICAL` → `steps/07-complexity-gate.md`.
