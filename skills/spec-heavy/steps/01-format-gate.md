...
description: "Spec Heavy: Format validation (25 checks) — full column table output"
...
# Spec Lifecycle (Heavy) — Step 01: Format Gate

> **Position in pipeline:** Step 1 — first quality gate. No spec content is reviewed here; only its FORMAT.
> **Goal:** Block specs that have structural defects before content review (step 02) and any implementation work begins. Heavy variant adds the BLOCK decision for specs with critical structural failures that make further analysis pointless.

---

## Quando usar

Use logo apos a spec estar escrita em `.kiro/specs/<feature>/` e antes de qualquer outro step do pipeline Heavy. Este passo e barato, deterministico e impede que problemas formais (campos faltando, IDs duplicados, sem coverage matrix) contaminem a fase de content review (step 02), implementation (step 04) ou post-impl (step 05).
...
| # | Check | Esperado |
|---|---|---|
| D1 | Cada task em formato checklist (`- [ ] Task X: ...`) | 100% das tasks |
...
## Formato de resposta obrigatorio (Heavy: tabela com header explicito)

```markdown
## FORMAT GATE REPORT (HEAVY) — [feature-name]
...
| A3  | design.md existe                       | 1     | PASS   |
| A4  | tasks.md existe                        | 1     | PASS   |
| A5  | research.md ou flag explicita          | 1     | PASS   |
| B1  | User Story em cada requisito           | N     | PASS   |
| ... | ...                                    | ...   | ...    |
