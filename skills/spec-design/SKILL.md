...
- `pipeline-runs/<run_id>/01-spec/spec.json`, `requirements.md`, `design.md` (if exists)
- `pipeline-runs/<run_id>/01-spec/research.md` (if exists, contains gap analysis from `/pipeline-orchestrator:validate-gap`)
...
- `references/spec-templates/design.md` for document structure
- Read `rules/design-principles.md` from this skill's directory for design principles
- `references/spec-templates/research.md` for discovery log structure
...
4. **Persist Findings to Research Log**:
   - Create or update `pipeline-runs/<run_id>/01-spec/research.md` using the shared template
...
1. **Write Final Design**:
   - Write `pipeline-runs/<run_id>/01-spec/design.md` only after the design review gate passes
...
1. **Status**: Confirm design document generated at `pipeline-runs/<run_id>/01-spec/design.md`
...
**Note**: The actual design document follows `references/spec-templates/design.md` structure.
...
- **Suggested Action**: "Run `/pipeline-orchestrator:spec-design {feature} -y` to auto-approve requirements and proceed"

**Missing Requirements**:
- **Stop Execution**: Requirements document must exist
- **User Message**: "No requirements.md found at `pipeline-runs/<run_id>/01-spec/requirements.md`"
- **Suggested Action**: "Run `/pipeline-orchestrator:spec-requirements {feature}` to generate requirements first"

**Template Missing**:
- **User Message**: "Template file missing at `references/spec-templates/design.md`"
...
- **Suggested Action**: Clarify or fix `requirements.md`, then re-run `/pipeline-orchestrator:spec-design {feature}`

### Next Phase: Task Generation

**If Design Approved**:
- **Optional**: Run `/pipeline-orchestrator:validate-design {feature}` for interactive quality review
- Run `/pipeline-orchestrator:spec-tasks {feature}` to generate implementation tasks
- Or `/pipeline-orchestrator:spec-tasks {feature} -y` to auto-approve and proceed directly

**If Modifications Needed**:
- Provide feedback and re-run `/pipeline-orchestrator:spec-design {feature}`
