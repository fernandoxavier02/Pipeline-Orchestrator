...
### Error Scenarios
- **Missing Requirements**: If requirements.md doesn't exist, stop with message: "Run `/pipeline-orchestrator:spec-requirements {feature}` first to generate requirements"
...
- Run `/pipeline-orchestrator:spec-design {feature}` to create technical design document
- Or `/pipeline-orchestrator:spec-design {feature} -y` to auto-approve requirements and proceed directly
