...
### Error Scenarios
- **Missing Design**: If design.md doesn't exist, stop with message: "Run `/pipeline-orchestrator:spec-design {feature}` first to generate design document"
...
- Re-run `/pipeline-orchestrator:spec-design {feature}` with improvements
- Re-validate with `/pipeline-orchestrator:validate-design {feature}`
