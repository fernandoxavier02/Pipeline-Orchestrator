...
test('Gate Registry section contains 49 data rows (Core 20 + Spec 6 + Routing 4 + Brainstorm 7 + Spec-authoring 8 + Spec-authoring-enforcement 4) post-v8.8.0 Refactor slice', () => {
  ...
  // so the registry was 47 + 1 = 48.
  // 48 → 49: +1 REFACTOR_SCOPE_LOCK (v8.8.0 Refactor slice, Core 19→20, HARD,
  // appended after PROTOCOL_HANDSHAKE_TIMEOUT). The Mandatory Gates by
  // Complexity table is UNCHANGED (REFACTOR_SCOPE_LOCK is workflow-specific,
  // not mandatory-by-complexity).
  const content = fs.readFileSync(TARGET, 'utf8');
  const rows = countDataRowsBetween(content, /^##\s+Gate Registry\s*$/m, /^##\s+Mandatory Gates by Complexity\s*$/m);
  assert.equal(rows, 49, `expected 49 data rows in Gate Registry section, got ${rows}`);
});
