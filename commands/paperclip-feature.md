---
description: "Despacha uma Feature para os robôs do Paperclip (VPS). Classifica complexidade localmente, mostra a árvore em dry-run e pede confirmação antes de criar qualquer cartão."
...
Este comando monta e despacha uma árvore de tarefas do tipo **Feature** direto nos robôs do Paperclip rodando na VPS, via túnel SSH. O Claude só faz a montagem; a execução acontece nos agentes da empresa Pipeline Orchestrator no servidor remoto.

**Empresa fixa:** Pipeline Orchestrator (PIP) — `019c6f31-41ed-497a-93ba-cb4eff651a7c`
**Tipo fixo:** `Feature`
**Variante (determinada pela complexidade):**
- `SIMPLES` → `feature.light`
- `MEDIA`   → `feature.light`
- `COMPLEXA` → `feature.heavy`

**O que o fluxo Feature faz:**
- `feature.light` — funcionalidade nova de complexidade baixa a média. Inclui: clarificação, quality-gate-router (cenários TDD), pre-tester (testes RED), plan-architect + aprovação, implementação por slices (batch 2-3), checkpoint, adversarial por lote (review-orchestrator + executor-fix), sanity-checker, trio adversarial final (segurança ‖ arquitetura ‖ qualidade) e fechamento com PA de Cal + SLICE_CLOSEOUT. **Sem** design-interrogator e **sem** feature-integration-validator. Total: ~20 cartões.
- `feature.heavy` — funcionalidade nova complexa ou de alto impacto. Acrescenta: design-interrogator (interrogação de decisões de design antes de planejar), feature-vertical-slice-planner (VSA_PLAN com aprovações de UX, arquitetura e plano), implementação slice a slice (batch 1), feature-integration-validator após cada slice, revisão em paralelo (architecture-reviewer ‖ diff-discipline-reviewer) antes do checkpoint, e trio adversarial final completo. Total: ~24 cartões.
...
```
Uso: /pipeline-orchestrator:paperclip-feature <descrição da funcionalidade>
Exemplo: /pipeline-orchestrator:paperclip-feature "adicionar exportação de relatório em PDF no painel de auditoria"
...
O tipo já é fixo (`Feature`). O que falta é a **complexidade**, que determina se vai para o fluxo `light` (SIMPLES ou MEDIA) ou `heavy` (COMPLEXA).
...
const result = classify(desc, { type: 'Feature' });
console.log(JSON.stringify(result, null, 2));
" -- "<descrição da feature sem flags>"
```

Resultado esperado: `{ type: 'Feature', complexity: 'SIMPLES'|'MEDIA'|'COMPLEXA', source: '...', notes: [...] }`

**Mapeamento para variante grow-tree:**
- `SIMPLES` → argumento `feature.light`
- `MEDIA`   → argumento `feature.light`
- `COMPLEXA` → argumento `feature.heavy`
...
AskUserQuestion
  ...
  options:
    - label: "SIMPLES — light (Recomendado pelo classificador)"
      description: "1-3 arquivos, funcionalidade isolada, sem impacto em arquitetura ou integrações externas"
    - label: "MEDIA — light"
      description: "4-6 arquivos, lógica moderada, toca mais de um módulo mas sem redesign"
    - label: "COMPLEXA — heavy"
      description: "7+ arquivos, design-interrogator obrigatório, integração entre slices, validação de integração por slice, adversarial completo"
```
...
_(Substitua `feature.light` por `feature.heavy` se COMPLEXA.)_
...
- Quantos cartões o fluxo tem no total (light ≈ 20 cartões, heavy ≈ 24 cartões)
...
- O Paperclip orquestra a fila de dominó: cada robô que conclui cria o cartão seguinte via `blockedByIssueIds`. O fluxo light tem ~20 cartões e o heavy ~24 — nenhum precisa de intervenção manual entre os passos.
