...
| **Versão canônica** | `.claude-plugin/plugin.json` `"version"` | Atualmente: **4.8.0**. Lineage: v4.2.0 → v4.2.1 → v4.3.0 → v4.3.1 → v4.4.0 (Slice 1.5 — Pulsar bugfix workflow import) → v4.4.1 (exec-window wrapper fix) → v4.5.0 (Slice 3a — Pulsar audit workflow import) → v4.6.0 (Slice 3b — Pulsar feature workflow import, single-skill-com-mode-flag pattern, REVERTIDO) → v4.6.1 (Slice 3b corrigido — 2 skills separadas espelhando Pulsar Heavy/ + Ligth/ 1:1) → v4.7.0 (Slice 3b polish — frontmatter contract completo + thin entry-point /feature + tests reais) → v4.8.0 (Hook Enforcement — sentinel_checkpoints/agent_type/gates_at enforced via shared parser + 3 hooks estendidos; warn mode até 2026-05-17, deny depois; resolve §17.4 #8). Marketplace lê este arquivo. |
