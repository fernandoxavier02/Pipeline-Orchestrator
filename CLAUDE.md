...
| **Versão canônica** | `.claude-plugin/plugin.json` `"version"` | Atualmente: **4.17.0**. Lineage: v4.2.0 → v4.2.1 → v4.3.0 → v4.3.1 → v4.4.0 (Slice 1.5 — Pulsar bugfix workflow import) → v4.4.1 (exec-window wrapper fix) → v4.5.0 (Slice 3a — Pulsar audit workflow import) → v4.6.0 (Slice 3b — Pulsar feature workflow import, REVERTIDO) → v4.6.1 (Slice 3b corrigido) → v4.7.0 (Slice 3b polish) → v4.8.0 (Hook Enforcement) → v4.9.0 (Spec Workflow Wave 2) → v4.9.1 (Spec polish) → v4.9.2 (CI debt) → v4.10.0 (Wave 3 Compat Baselines) → v4.11.0 (Wave 3-spec) → v4.12.0 (Wave 4-spec) → v4.13.0 (Wave 5-spec) → v4.14.0 (Wave 6-spec) → v4.15.0 (Wave 7-spec, MIGRATION + readiness report) → v4.16.0 (Wave 8-pre, Issue #11 real-mode compat runner) → **v4.17.0 (Wave 8-spec — TRACE.md schema + writer + standalone validator + plan-mode auto-trigger on MEDIA/COMPLEXA + --no-plan bypass with COMPLEXA override-blocked semantics; +17 tests; closes DoD #2 #3 #6; self-hosted dogfood revealed sentinel-hook cwd-discovery brittleness for v5.0-rc.1 follow-up)**. Marketplace lê este arquivo. |
