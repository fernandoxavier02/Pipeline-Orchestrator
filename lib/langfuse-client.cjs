...
/**
 * isEnabled() — opt-in gate. PURE predicate: no I/O on the happy path
 * beyond writing one missing_credentials log entry the first time creds
 * are absent (which is itself the gate's verdict, not a diagnostic).
 *
 * Returns true ONLY when:
 *   process.env.LANGFUSE_ENABLED is exactly "true" or "1" (case-insensitive)
...
function isEnabled() {
  ...
  const flag = String(process.env.LANGFUSE_ENABLED || '').toLowerCase();
