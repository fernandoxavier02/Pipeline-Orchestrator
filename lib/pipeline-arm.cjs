...
// DATA, not a user command. An agent report that merely MENTIONS "review-only"
// must NEVER arm a phantom run. v8.20.0-glm port: paperclip removed; the guard
// stays as defense against any future flag referenced in agent text.
//
...
// v8.20.0 (F4): + teammate-message / agent-message — a background subagent's
// report quoting "/pipeline-orchestrator:pipeline continue" in prose arrived as a
// prompt turn wrapped in these envelopes and armed a phantom CONTINUE run that
// bricked the session's Bash/PowerShell/Agent until the TTL (live 2026-07-02).
const HARNESS_OPEN_TAG =
  /<(?:task-notification|system-reminder|teammate-message|agent-message|function_results|function_calls|antml:function_calls|result|system|attachment|bash-(?:input|stdout|stderr)|tool_use_error)\b/i;
...
function writeArmPending(cwd, rawPrompt, nowIso, sessionId) {
  ...
  // A read-only TYPE (Audit, UX Simulation) that was INFERRED FROM KEYWORDS — not an
  // explicit --audit/--ux flag — is almost always a casual natural-language "faça
  // revisão/análise/auditoria". Arming it created the recurring phantom-arm deadlock
  // (interview-coach 2026-06-30 / 07-01: read-only run never opens an exec window →
  // INLINE_WORK_BLOCKED → abort → orphan marker bricks the next task). An EXPLICIT
  // --audit/--ux flag is a deliberate governed run and STILL arms (it carries the
  // STEP_1_7 direct-entry evidence, v8.16.0). Safe direction per the arm trade-off
  // above: a missed arm is recoverable (re-invoke); a phantom arm deadlocks.
  if ((wf.type === 'Audit' || wf.type === 'UX Simulation') &&
      typeof wf.source === 'string' && wf.source.startsWith('kw:')) return null;
  const marker = {
