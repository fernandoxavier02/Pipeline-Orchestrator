...
const MODES = new Set(['FULL', 'DIAGNOSTIC', 'CONTINUE', 'HOTFIX', 'REVIEW-ONLY']);
...
// plugin command is present. Meta/read-only commands (help) are DELIBERATELY
// excluded: arming them would freeze their own read-only work. Explicit allowlist,
// not a loose `^/pipeline` prefix (which both anchored to start AND re-armed
// the meta cmds). v8.20.0-glm port: paperclip-* commands removed from allowlist.
const GOVERNED_INVOCATION = /\/pipeline-orchestrator:(?:pipeline|bugfix|feature|audit|user-?story|ux(?:-sim)?|refactor|spec|review|brainstorm)(?:-light|-heavy|-audit-only|-design|-init|-requirements|-tasks|-review)?\b/;
...
function classifyWorkflow(rawPrompt) {
  ...
  // v8.20.0-glm port: --on=paperclip flag removed.
