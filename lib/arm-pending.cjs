...
function readArmPending(dir, sessionId) {
  ...
  // v8.18.0: session isolation — se sessionId fornecido E o marker tem session_id,
  // só retorna armed se a sessão bate. Marker legado (sem session_id) → armed para todos.
...
function readArmPendingMarker(dir, sessionId) {
  let m;
