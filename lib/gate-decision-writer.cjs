...
function appendGateDecision(filePath, entry, ctx) {
  ...
  // Clock-skew hardening: LLM callers (controllers/agents) have no reliable clock,
  // so a caller-supplied `timestamp` routinely drifts minutes-to-hours from the real
  // write time, tripping the C1 backdating detector in audit-forgery-guard.cjs with
  // false "backdated" blocks (and, symmetrically, future-stamping that breaks skew
  // windows). This writer runs as code with a real clock, so IT is the authority for
  // the timestamp: stamp real write-time — EXCEPT for a deliberate backfill, where the
  // caller's timestamp is intentional and carries the sanctioned BACKFILLED marker the
  // guard already honors.
  const isBackfill =
    typeof entry.detail === 'string' && entry.detail.includes('BACKFILLED');
  const stampedTimestamp =
    isBackfill && typeof entry.timestamp === 'string' && entry.timestamp
      ? entry.timestamp
