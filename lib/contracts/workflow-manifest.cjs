...
// in-code spine. Only FULL carries a spine today (the 9 governed controllers), so this
// is effectively "is one of the 9 FULL leaves", but written workflow-agnostic so that
// adding a non-FULL spine later extends it automatically with no hook change. This is
// the membership test the observer uses to reject non-pipeline harness agents
...
//
// SCOPE NOTE: "governed leaf" HERE means a result-block-obligated FULL spine leaf (the 9
// that emit PIPELINE_AGENT_RESULT_V1). This is INTENTIONALLY narrower than step-ledger's
