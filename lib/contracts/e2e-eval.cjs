...
const HYGIENE_EVENT_SEVERITY = Object.freeze({
  ...
  // AUDIT-004 (v8.17.0): a run executed end-to-end with the canonical
  // execution-gate hook missing from the install. Highest possible weight —
  // a run with enforcement INACTIVE cannot reach a clean ProcessHygiene score
  // no matter how green the rest of the trace looks (the rest of the trace is
  // not trustworthy when the porter was absent).
  ENFORCEMENT_INACTIVE: 1.0,
 ...
 *                          v8.20.0 (F1): a counter bump that cannot be PERSISTED
 *                          also allows (fail-open) — an unadvanceable counter can
 *                          never reach the cap, so blocking would loop forever.
 */
