...
// buildSubagentFallbackClause(blockType) — Batch 3: the SAME escape-hatch
// sentence resolveNextAction already builds into subagent_fallback_action,
// factored out so hooks whose deny shape ISN'T the workflow-step model (e.g.
// dispatch-pending-gate.cjs, which already knows exactly which handshake type
// is pending) can append it to their OWN reason text without forcing their
// context into resolveNextAction's ctx shape. blockType names the SPECIFIC
// handshake the caller already knows is relevant; defaults to DISPATCH_REQUEST
// (the shape ALL three collapse to for a subagent reader — AskUserQuestion and
// EnterPlanMode are unavailable to it regardless of which type is pending,
// Achado #7, so its only real exit is always "ask the parent via a block").
const VALID_BLOCK_TYPES = new Set(['DISPATCH_REQUEST', 'GATE_REQUEST', 'PLAN_MODE_REQUEST']);
function buildSubagentFallbackClause(blockType) {
  const bt = typeof blockType === 'string' && VALID_BLOCK_TYPES.has(blockType) ? blockType : 'DISPATCH_REQUEST';
  return (
    `Se você é um subagente sem a ferramenta Agent/AskUserQuestion/EnterPlanMode disponível ` +
    `(Achado #7): emita agora um bloco ${bt} (ver references/gate-request-protocol.md) e encerre ` +
    `o turno — o processo pai processa e retoma você.`
  );
}
