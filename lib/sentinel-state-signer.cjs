 ...
 * Returns a DURABLE hex/secret string, or null when no durable key can be
 * persisted.
 *
 * R5 (H1-002 / SEC-1 fix): when the key file cannot be written (read-only home,
 * container CI, permission change), this returns null INSTEAD of fabricating a
 * per-process ephemeral (pid+Date.now()) key. The old ephemeral fallback bricked
 * every governed run: state signed by one process verified false in the next, got
 * classified CORRUPT, and the dispatch gate DENIED every Agent spawn. With null,
 * signState() writes the state UNSIGNED (the established `if (key) signState`
 * convention live at lib/pipeline-arm.cjs), which verifiers treat as
 * UNSIGNED-TOLERATED (allowed unless PIPELINE_HMAC_STRICT) — never CORRUPT.
 */
function getOrCreateHmacKey() {
  ...
  try {
    ...
    // No DURABLE key available. Return null so signing contexts fall through to
    // the UNSIGNED-TOLERATED path (migration convention) rather than fabricating a
    // throwaway key that bricks the next process's verification.
    process.stderr.write(
      `[SENTINEL-SIGNER WARN] cannot persist a durable HMAC key (${err.message}); ` +
      'writing state UNSIGNED (migration-tolerated unless PIPELINE_HMAC_STRICT)\n'
    );
    return null;
  }
}
...
/**
 * Sign a state object — returns a new object with the `__signature` field, OR,
 * when no durable key is available, returns the state UNSIGNED (no `__signature`).
 *
 * R5 (H1-002 / SEC-1 fix): when the resolved key is null (no durable key could be
 * persisted) we write the state UNSIGNED instead of throwing or signing with a
 * fabricated ephemeral key. This mirrors the established contract already live at
 * lib/pipeline-arm.cjs (`if (key) signState(...)`): a state with no signature is
 * UNSIGNED-TOLERATED by verifyState (valid unless PIPELINE_HMAC_STRICT), NOT
 * CORRUPT. A genuinely TAMPERED signed state (signature mismatch against a real
 * persisted key) still verifies false and still denies.
 */
function signState(stateObj, key) {
  ...
  if (!hmacKey) {
    // No durable key → emit the state UNSIGNED (migration-tolerated). Strip any
    // stale __signature a caller may have carried in so we never ship a signature
    // that no key backs.
    const { [SIGNATURE_FIELD]: _stale, ...unsigned } = stateObj;
    return unsigned;
  }
