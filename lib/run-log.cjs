...
function normalizeEntry(entry) {
  ...
  for (const field of REQUIRED_FIELDS) {
    ...
    if (field === 'total_gates_triggered' || field === 'total_gates_expected'
      ...
      // R10.3: an UNKNOWN value stays null (honest), never a misleading 0 — so
      // "coverage/duration unknown" and "genuinely zero" remain distinguishable
      // downstream (e.g. the watchdog telling unknown coverage from full coverage).
      // A real number (or numeric string) is coerced; a non-finite value → null.
      // This completes the v7.9.1 D4 fix that normalizeEntry was re-coercing away.
      if (value == null) {
        value = null;
      } else {
...
function appendRunLog(repoRoot, entry) {
  ...
  // SEC-5 + v7.13.0 (R8): exclusive file-lock around the append so two concurrent
  // appenders cannot interleave bytes on a single line. Now shared with the JSONL
  // writers via lib/exclusive-lock.cjs, which adds STALE-ORPHAN RECOVERY the old
  // inline mutex lacked — a crashed appender's lock no longer blocks future writes.
  try {
    withLock(file, () => {
