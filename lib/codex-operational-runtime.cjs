...
// v7.14.0 (B1): STEP 1.7 branch identity is a CLOSED 4-value vocabulary; the
// helper validates the branch and maps it to the canonical gate-decision value
// (D1) so the Codex path can never emit an out-of-vocabulary branch (the bug the
// 2026-06-12 audit caught). Shared SSOT with the Claude controller path.
const { appendStep17Routing } = require('./step-1-7-routing.cjs');
// v8.27.0 (Requirement 9 — UserGateSignature): user-gate protocol-events (the
// Achado #7 GATE_REQUEST/GATE_RESPONSES handshake) are HMAC-signed at write time
// so the forgery guard's C3 corroboration can no longer be fooled by a fabricated
// user-gate event. Scope restricted to user-gate events (isUserGateEvent).
const { signUserGateEvent, isUserGateEvent } = require('./protocol-event-signer.cjs');
...
function appendJsonl(file, entry) {
  ...
  try {
    // v8.27.0 (Requirement 9.1): sign USER-GATE events only (scope-restricted);
    // every other protocol-event passes through unchanged. Two-tier: no durable
    // key → the signer returns the event UNSIGNED (migration-tolerated).
    const toWrite = isUserGateEvent(entry) ? signUserGateEvent(entry) : entry;
    safeAppendJsonl(file, toWrite, { kind: 'protocol' });
  } catch (err) {
...
async function runOperationalPipeline(options = {}) {
  ...
  // SEC-RR-1: the anchored createPipelineDocPath() branch is always inside cwd
  // and safe. A caller-supplied options.pipelineDocPath, however, is later used
  // to mkdir + append JSONL audit files, so a "../" value could write outside
  // the run sandbox. Resolve it and verify containment within cwd (mirrors the
  // separator-boundary prefix check in lib/fidelity-reporter.cjs:377-386). If it
  // escapes, reject by falling back to the anchored path rather than writing.
  let pipelineDocPath = createPipelineDocPath(cwd, task || mode);
  if (options.pipelineDocPath) {
    const resolvedDoc = path.resolve(cwd, options.pipelineDocPath);
    const cwdWithSep = cwd.endsWith(path.sep) ? cwd : cwd + path.sep;
    if (resolvedDoc === cwd || resolvedDoc.startsWith(cwdWithSep)) {
      pipelineDocPath = resolvedDoc;
    } else {
      try {
        process.stderr.write(
          `runOperationalPipeline: pipelineDocPath "${options.pipelineDocPath}" escapes cwd containment → falling back to anchored path\n`
        );
      } catch (_e) { /* */ }
    }
  }
  ...
  let status = 'RUNNING';
  let reason = null;
  // SEC-2: brainstormConcluded must NOT be inferred from a substring scan of LLM
  // output — prose merely mentioning 'brainstorm-controller' / 'STEP_1_7_ROUTING'
  // would flip it true before any gate is processed (concluded:true with no
  // evidence). Initialize FALSE unconditionally; the only place it becomes true is
  // an ACTUAL STEP_1_7_ROUTING gate entry at the gate-processing sites below.
  let brainstormConcluded = false;
  let iterations = 0;
    ...
    for (const block of blocks) {
      if (block.kind === 'DISPATCH_REQUEST') {
        ...
        if (block.payload.target_name === 'brainstorm-controller') {
          // v7.14.0 (B1): the branch is FIXED here — dispatching the
          // brainstorm-controller IS, by definition, the `dispatch-brainstorm`
          // branch. (ARCH-B1-003: the old `if (!BRANCH_VALUES.has(branch))`
          // guard was dead code against a hardcoded literal and was removed.)
          //
          // ARCH-B1-001/SEC-2: DELEGATE the STEP_1_7_ROUTING entry to the shared
          // helper (single source of the 7-field schema + branch validation +
          // detail sanitization). Passing prep_run_id through the helper applies
          // the SEC-1 charset allowlist + JSON.stringify, so the LLM-generated
          // dispatch_id can no longer forge the structured detail record. The ctx
          // (run_id/type/complexity correlation envelope) is built exactly as
          // logGateDecision does so the audit row stays correlated.
          //
          // ARCH-B1-002: this is the ONLY STEP_1_7_ROUTING write site on the
          // Codex path BY DESIGN — it logs the dispatch-brainstorm branch. The
          // other three branches (load-existing / no-prep-override /
          // simples-bypass) are decided and logged by the controller, not by
          // this runtime loop, so they intentionally have no write site here.
          // D3 responsibility split: writing the sentinel-state step_1_7 block
          // (buildStep17StateBlock) is the CONTROLLER's duty, performed before
          // any Phase 1.5 / Phase 2 spawn. This Codex runtime ONLY logs the gate
          // decision for the dispatch-brainstorm branch — it does not touch
          // sentinel-state. (phase '1.7' is the helper default but is passed
          // explicitly here for consistency with the other write sites.)
          appendStep17Routing(
            path.join(pipelineDocPath, 'gate-decisions.jsonl'),
            {
              branch: 'dispatch-brainstorm',
              prep_run_id: block.payload.dispatch_id || block.payload.id,
              phase: '1.7',
              decided_by: 'runtime',
              timestamp: nowIso(),
            },
            buildCtx(pipelineDocPath, {
              type: session && session.options ? session.options.type : null,
              complexity: session && session.options ? session.options.complexity : null,
            }),
          );
