...
// pivot a table cell.
const VALID_HARDNESS = new Set(['MANDATORY', 'HARD', 'CIRCUIT_BREAKER', 'SOFT']);
const VALID_DECISION = new Set([
  'TRIGGERED', 'NOT_TRIGGERED', 'SKIPPED',
  'APPROVED', 'CONFIRMED', 'REJECTED',
]);
const VALID_COMPLEXITY = new Set(['SIMPLES', 'MEDIA', 'COMPLEXA']);

function validateHardness(v) {
  if (v == null) return null;
  if (typeof v !== 'string') return '(invalid)';
  return VALID_HARDNESS.has(v) ? v : '(invalid)';
}
function validateDecision(v) {
  if (v == null) return null;
  if (typeof v !== 'string') return '(invalid)';
  return VALID_DECISION.has(v) ? v : '(invalid)';
}
...
function generateFidelityReport(opts) {
  try {
    ...
    for (const e of entries) {
      ...
      const record = {
        hardness: validateHardness(e.hardness),
        decision: validateDecision(e.decision),
      };
