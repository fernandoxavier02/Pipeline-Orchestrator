...
4. If the subagent's tool result ended with `STATUS: AWAITING_GATE_RESPONSES` (or AWAITING_DISPATCH_RESULTS, AWAITING_PLAN_MODE_RESULTS), re-dispatch the subagent with the original prompt + the response payloads prepended. **"Re-dispatch" means a FRESH `Agent()` call** — the subagent re-hydrates its state from the run-dir on disk (manifest + `notes.options` + artifacts), not from in-memory continuity. "SAME subagent" means the same `subagent_type` + same `run_id`, NOT the same live process.
...
> **Anti-pattern — do NOT reach for `SendMessage` / agent-resume.** This protocol is **stateless per dispatch by design**: every `AWAITING_*` is closed by a fresh `Agent()` re-dispatch whose state lives in the run-dir, never by continuing a live agent. `SendMessage` (and any "resume the same agent with its memory intact" mechanism) is **not used anywhere** in this pipeline and is **not required**. If you (the parent) conclude you need `SendMessage` to hand a result back to a waiting subagent, you have mis-read the protocol — re-dispatch instead. (For the record: even where `SendMessage` exists, it is a deferred tool loadable via `ToolSearch`; its apparent absence is never a reason to abandon a run.)
