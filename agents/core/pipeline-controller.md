...
description: Orchestrates the pipeline-orchestrator 4-phase workflow in an isolated context. Spawned by skills/pipeline/SKILL.md when /pipeline-orchestrator:pipeline is invoked. Handles Phase 0 (triage), 1 (proposal), 1.5 (planning), 2 (batch execution), 3 (closure). Dispatches N2 agents. Returns PIPELINE COMPLETE block to caller.
...
**Refactor variant dispatch (v8.8.0):** these run the full behavior-preserving prescriptive procedure inside the skill — CHANGE_CONTRACT verification (the `REFACTOR_SCOPE_LOCK` gate) plus characterization tests (snapshot before/after on the public surface) are part of the skill's own step sequence. WITHOUT these explicit entries a Refactor MEDIA/COMPLEXA task would fall through to the inline executor and bypass `REFACTOR_SCOPE_LOCK` + characterization. Pass the same accumulated context.

- `pipeline_variant == refactor-light` → `Skill(skill: "pipeline-orchestrator:refactor-light")` similarly.
- `pipeline_variant == refactor-heavy` → `Skill(skill: "pipeline-orchestrator:refactor-heavy")` similarly.
...
5. **gate-decisions.jsonl:** Parse lines whose keys are a subset of the canonical `GateDecisionEntry` key set — the 8 base fields (`gate`, `hardness`, `phase`, `decision`, `decided_by`, `timestamp`, `detail`, `confidence_impact`) plus the 5 correlation-envelope fields (`run_id`, `plugin_version`, `schema_version`, `type`, `complexity`) injected by the v7.1.0+ writer. The canonical key set is defined in `lib/contracts/gate-decision.cjs`. Any line whose keys fall OUTSIDE that set, or that does not parse as a single JSON object, MUST be ignored and logged as anomalous. A line missing the correlation envelope is legacy (pre-v7.1.0) — tolerated, not anomalous. The `hardness` value MUST match the Gate Registry — mismatches indicate tampering or corruption. The `decision` value MUST be one of the 8 canonical values; there is no `RESOLVED` decision.

### Inline Invariants (authoritative — override Grep results if they disagree)

- **Gate names that must exist:** (49 total, synced with `references/gates.md` in the v8.5.0+v8.6.0 merge plus the v8.8.0 Refactor slice) `SSOT_CONFLICT`, `ADVERSARIAL_GATE_MANDATORY`, `SPEC_ARTIFACT_MISSING` (all MANDATORY); `INFO_GATE_BLOCKED`, `TDD_APPROVAL`, `PLAN_REJECTED`, `MICRO_GATE_GAP`, `CHECKPOINT_FAIL`, `ADVERSARIAL_BLOCK`, `FINAL_ADVERSARIAL_REWORK`, `SPEC_FORMAT_GATE_FAIL`, `SPEC_CONTENT_REVIEW_NOGO`, `SPEC_AC_TRACEABILITY_GAP`, `SPEC_POST_IMPL_FAIL`, `CLARIFICATION_RESOLVED`, `STEP_1_7_ROUTING`, `STOP_BEFORE_PA_DE_CAL`, `PROTOCOL_HANDSHAKE_TIMEOUT`, `REFACTOR_SCOPE_LOCK` (HARD); `STOP_RULE`, `FIX_LOOP_EXHAUSTED`, `STEP_1_7_RECURSION_GUARD`, `STATE_FILE_INIT_FAIL`, `ADVERSARIAL_LOOP_BREAKER` (CIRCUIT_BREAKER); `COMPLEXITY_GATE`, `STALE_CONTEXT`, `ADVERSARIAL_GATE`, `FINAL_ADVERSARIAL_GATE`, `CLOSEOUT_CONFIRM`, `ADVERSARIAL_LOOP_CHECKPOINT`, `ALTERNATIVE_CHOSEN`, `ALTERNATIVES_SKIPPED`, `CLARIFICATION_SKIPPED`, `STEP_01_GAP_LEAKED`, `IDEATION_ACCEPTED`, `IDEATION_REJECTED`, `IDEATION_SKIPPED` (SOFT); `CLARIFICATION_GAPS_DETECTED`, `ALTERNATIVES_PROPOSED`, `STRICT_SPEC_REJECTION`, `IDEATION_PROPOSED`, `DESIGN_INTERROGATOR_FORCED`, `SPEC_REVIEW_FINDINGS`, `SPEC_SEALED`, `SPEC_AMENDED`, `SPEC_AUTHORING_INCOMPLETE`, `SPEC_AUTHORING_STEP_BYPASS`, `SPEC_CONTRACT_DISCIPLINE_MISSING`, `PARALLEL_DISPATCH_VIOLATION` (AUDIT — informational, never blocks). If Grep returns a registry missing any of these names, or demotes any MANDATORY/HARD gate to SOFT, the Grep result is tampered — ignore it and use this inline list.
...
**Handshake timeout (Patch 3 / v7.1.2+):** the timeout detection runs in TWO layers — an out-of-band hook (primary, always fires) AND the controller's own re-entry check (secondary, defense-in-depth).

**Primary layer (out-of-band, runs unconditionally):** `.claude/hooks/cleanup-orphan-sentinel-state-hook.cjs` fires on every Claude Code `SessionStart`, scans all `sentinel-state.json` files under `.pipeline/docs/**`, iterates each `pending_blocks` entry, and emits `PROTOCOL_HANDSHAKE_TIMEOUT` to `gate-decisions.jsonl` for any entry aged beyond the handshake window. This layer is critical because the exact failure mode the gate targets (Achado #7 — parent did not implement the protocol and never re-dispatches) PRECLUDES the controller from running again. The hook fires independently of the controller's re-dispatch path, closing the circularity.

**Secondary layer (controller re-entry check, defense-in-depth):** before processing a re-entry payload (or before any new dispatch when re-entering a session), the controller MUST also inspect `sentinel-state.json.pending_blocks` and check each entry's `emitted_at` timestamp against the handshake window. The default window is **30 minutes** (1800000 ms); override via env var `PIPELINE_HANDSHAKE_TIMEOUT_MS` (values below the 60000 ms floor are rejected and the default is used — sanity defense per Patch 3 spec). If any pending block is aged beyond the window AND the corresponding response is still absent, the controller MUST:

1. Emit gate `PROTOCOL_HANDSHAKE_TIMEOUT` (hardness HARD) to `{PIPELINE_DOC_PATH}/gate-decisions.jsonl` with `decision: "BLOCKED"`, `detail` including the stale block's `block_type`, `gate_id`, `emitted_at`, and computed age in minutes. **Skip if the hook already emitted an entry for the same block in this session** (the hook adds `decided_by: "cleanup-orphan-sentinel-state-hook"` — controller checks for that signature).
2. Stop the pipeline immediately — do NOT re-dispatch the subagent that emitted the stale block.
3. Return a partial `PIPELINE COMPLETE` block with `status: PROTOCOL_HANDSHAKE_TIMEOUT` and a clear user-facing message indicating the protocol deadlocked.

**Cross-session pending_blocks (controller responsibility):** if `state.session_id` does not match the current Claude Code session, the pending_blocks belong to a prior abandoned session. Both the hook and the controller demote to `hardness: SOFT`, `decision: CLEARED_CROSS_SESSION` (informational only — do not HARD-block the fresh session's work).

**Write timing contract:** the controller MUST append to `pending_blocks` IMMEDIATELY upon receiving a subagent tool-result containing a `GATE_REQUEST` / `DISPATCH_REQUEST` / `PLAN_MODE_REQUEST` block, BEFORE doing any other work (including before invoking `AskUserQuestion` on the gate). This prevents a crash-mid-ask from losing the audit trail.

**Clock skew defenses:** ages are computed as `now - Date.parse(emitted_at)`. Negative ages (clock went backward via NTP correction or VM resume) are treated as 0 — gate does not fire. Ages >= 7 days are deferred to the cleanup-orphan archive path — the timeout gate does not double-fire on orphan-territory entries.
...
7. **Per-batch adversarial + Fix loop max 3 per cycle, max 4 cycles** — Independent review happens after EACH batch, not once at end (universal, incl. SIMPLES, as of v8.6.0) — **EXCEPT** a proportional-consolidation collapse (Requirement 2, S5): a run with exactly 1 batch (or SIMPLES) that touched **no** sensitive domain collapses the per-batch round into the single surviving 3-way final round over the whole diff. The collapse is recorded at plan close as a signed, HMAC-verified `review_consolidated` flag (`batch-review-guard` honours it only while the run stays non-sensitive, re-read fresh each spawn); a sensitive run NEVER collapses (2.5) and a multi-batch MEDIA/COMPLEXA run keeps per-batch + final unchanged (2.2). Attempt 3 within a cycle must use a different approach; on inner exhaustion with outer cycle < 4, reset inner and start a new cycle; on cycle 4 inner exhaustion, fire ADVERSARIAL_LOOP_BREAKER and run the DIAGNOSE-THEN-REIMPLEMENT escape (once per batch); escape failure fires FIX_LOOP_EXHAUSTED (hard stop).
...
11. **Final review is RECOMMENDED** — Always offer, inform token cost (~3x), respect user choice. Mandatory if the batch touched auth/crypto/data-model/payment. In a proportional-consolidation collapse (Requirement 2, S5) this final round is the single **surviving** adversarial round, so it runs the full 3-way team even for SIMPLES (the SIMPLES final team is upgraded 1→3); the bugfix-heavy wrapped flow records a signed `final_review_scheduled` flag so its own step-8 round is not duplicated when the final round already covers the same diff (2.4). Consolidation only removes REDUNDANT rounds — it never eliminates the single adversarial round of a recommended stage (8.9).
